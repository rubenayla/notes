...
## One-directional link rule
This site is the personal-reference companion to the recruiter-facing **portfolio** (`rubenayla/portfolio` → https://rubenayla.xyz/). The link is **one-directional**: this site links *to* the portfolio (a `Portfolio ↗` nav entry), the portfolio never links here. That keeps the hiring-facing surface clean. Never add a link from portfolio → notes.
