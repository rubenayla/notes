...
Rejected the precise atomic alternatives (Rydberg / Bohr / electron Compton, ~10⁻¹⁰) in favour of maximal fundamentality over realizable precision, accepting that the unit's metre-value is only ~5 figures (G-limited): exact by definition, sharpening as G improves, and far beyond any human-scale need anyway. Kept for the record if precision ever trumps purity: (1/R∞) × 12⁶ = 272.09 mm, the harmon minus its legacy fudge.

**Refinement (same day) — the real reason for the Planck anchor.** The first-pass justification ("atomic anchors reference a contingent particle, Planck is purer") turned out to be weak and was reconsidered: the electron is fully universal (identical everywhere; R∞ known to ~10⁻¹²), and Planck is *not* less contingent — it leans on G, itself a free parameter of nature. Contingency is symmetric (gravity's coupling vs the electron's mass), so it can't be the deciding factor. The genuinely deciding reason is structural: **the Planck length is (expected to be) the minimal meaningful length** — below it the classical notion of distance breaks down (generalized uncertainty principle; discreteness in loop quantum gravity; a minimal length in string theory — the strong mainstream *expectation*, not an experimentally proven theorem). So a Planck-anchored system has a **natural floor**: its "1" (12⁰ = one Planck length) is the smallest physically meaningful length, and a length < 1 is physically impossible/meaningless. The Rydberg length has no such property (atoms and nuclei sit far below its 91 nm), so its "1" floats arbitrarily mid-spectrum. A unit system whose base coincides with nature's hard floor is the substantive (not merely aesthetic) reason to anchor on the Planck length. **This is the settled basis.**

**Everyday human rung: N=31 = 4.60 cm.** The N=32 rung (55 cm) exceeds a stretched hand span (~22 cm — big hands, near the human max), which feels awkward; the ergonomic rule adopted is "the everyday unit should not exceed the stretched hand, and stay within an order of magnitude of it." One hop down (N=31, 4.60 cm) is the only *fundamental* rung that satisfies it — it's finger-scale, so the body reference is a *part* of the hand, not the whole: ≈ index finger tip-to-middle-knuckle (50 mm in `clothing.md`, ~8% over). Its dozenal subdivision lands on a hand feature too: 1/12 unit = 3.84 mm ≈ the finger-segment mark (`clothing.md` "4 mm"). Body in these units: hand span ≈ 4.8, foot ≈ 6, reach ≈ 12 (one dozen), height ≈ 38. The Planck *floor* stays at N=0 (the system's true "1"); N=31 is just the convenient everyday rung. Note on the landscape: every *atomic* anchor (Rydberg/Bohr/Compton…) puts its human-scale rungs at ~27 cm and ~2.3 cm — clustered because α ≈ 1/144 = 12⁻² and α/4π ≈ 12⁻³ — so 27 cm (a foot) is the nearest atomic option but it *exceeds* the hand; Planck's 4.6 cm is the only fundamental rung that fits under it. A unit *at* whole-hand size (~22 cm) is reachable from no fundamental anchor without a forbidden tuned multiplier, so finger-scale is the honest best fit. Finger/thumb units have good historical precedent (inch = thumb, digit = finger-width).
...
## 2026-07-05 — DC connector page: the hard requirement is ONE connector, sized for the max, negotiated

The `ideas/power-delivery-dc-connector.md` page describes **one single connector** for all everyday home power — not a family of sizes. This is the load-bearing requirement; get it wrong and the whole idea collapses into "20 incompatible plugs," which is the thing it exists to replace.

**How one connector spans the whole range (the USB-C model, scaled up):**
- **One physical size**, built to the home maximum: **400 V, 25 A = 10 kW** (Ø4 mm centre pin, ~26 mm OD, insulated for 400 V — same insulation class as 230 V AC). Everything plugs into everything.
- **Voltage and current are negotiated electronically per device**, exactly like USB-C PD. The "two voltages" (48 V, 400 V) are **two negotiated states of the one connector, NOT two connectors.** A lamp negotiates 48 V at a fraction of an amp; a vacuum 400 V at 2.5 A; a kettle or home EV charge 400 V at up to 25 A — all through the same plug.
- **Dead-front keeps every state safe** regardless of what's negotiated. Dumb devices default to 48 V via a resistor on the handshake (CC) pin, USB-C style; 400 V only ever comes on after an active chip negotiates it.
- A small load just **over-provisions** the connector (a phone uses a 25 A-rated plug at 0.2 A), the way a wall socket today powers a phone charger. That's fine.
- **The one honest boundary:** public DC fast-charging at hundreds of amps exceeds what a coaxial connector can cool (~25 A / 10 kW ceiling), so *that* hands off to NACS. It's a different scale and location (a charging station), not a second home connector. Home EV charging ≤10 kW fits the one connector.
